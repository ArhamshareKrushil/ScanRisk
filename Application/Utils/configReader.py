...
import base64
import json
from Application.Utils import cst
#######################################################################################################################
...
config_location =os.path.join( loc1[0],  'Resources','config_json.json')

#######################################################################################################################
def readConfig_All():
    try:
        print("ssss:",config_location)
...
def read_broker(self):
    f1 = open(config_location)
    jConfig = json.load(f1)
    f1.close()
    # broker = jConfig['Broker']
    # return broker
...
def writeAPIdetails(token,usertype,Type_id):
    try:
        f1 = open(config_location)
        jConfig = json.load(f1)
        f1.close()
        ...
        jConfig['FastApiToken']['Token'] =token
        jConfig['FastApiToken']['Type'] =usertype
        jConfig['FastApiToken']['Type_id'] =Type_id


        jConfig['FastApiToken']['login_time']= current_time
        jConfig['FastApiToken']['login_date']= current_date
...
def readLoginId():
    try:
        ...
        loginid = jConfig['MDToken'] ['loginid']
        print("login:", print("jConfig:", jConfig))
        f1.close()
...
def read_API_config(self):
    ...
    self.FastApiURL = jConfig['FastApiURL']
    self.token = jConfig['FastApiToken']['Token']
    self.UserType=jConfig['FastApiToken']['Type']
