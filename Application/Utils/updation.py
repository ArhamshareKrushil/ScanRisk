...
def updateCMPOTWpos(main,data):
    ...
    main.CMPOTW.table[main.CMPOTW.model.lastSerialNo] = [data[0],
                                                     'NSECM', data[18], data[17], data[16], 0.0,
                                                     0.0, 0.0, 0.0, main.CMPOTW.lastSerialNo, Qty, Amt,Qty,
                                                     Amt,0.0]
    ...
    main.RecieverCM.subscribedlist('CMPOTW', 'NSECM', data[18])
...
def updatePOTW(main,data):
    ...
    if (data[1] in ['OPTSTK', 'OPTIDX']):
        netPrem = -(data[17]*data[7])
    else:
        netPrem = 0.0
    ...
    # print('Oamt',Oamt,'data',data[17])


    main.POTW.table[main.POTW.model.lastSerialNo] =[data[0], 'NSEFO', data[14], data[1], data[2], data[3],
                                                        data[4], data[5], 0, 0.0, 0,
                                                        0, main.POTW.model.lastSerialNo, data[7], Oamt, data[7], Oamt, data[10], data[11],
                                                        data[12],netPrem,0.0,0.0,0.0]
    ...
    # print('sssssssssss',ind.data(),main.POTW.model.rowCount(''),main.POTW.model.columnCount(''))


    main.POTW.model.dataChanged.emit(ind, ind1)

    main.Reciever.subscribedlist('POTW','NSEFO',data[14])

    main.isPOTWupdated = True

    if data[0] not in  main.POTW.clientList:
        main.POTW.clientList.append(data[0])


def updatePOTM(main,data):
    # print('in update POTM',data)
    ...
    main.POCW.table[main.POCW.model.lastSerialNo] = [data[1],
                                                   'NSEFO',data[12],data[13],data[2],data[3],
                                                               data[4],data[5],0,0.0,data[10],
                                                   0.0,main.POCW.lastSerialNo,data[11],openAmt,data[11],
                                                                                   openAmt,0.0,0.0,netPrem,0.0]
    ...
    main.Reciever.subscribedlist('POCW', 'NSEFO', data[12])



    main.isPOCWupdated=True

    if data[1] not in  main.POCW.clientList:
        main.POCW.clientList.append(data[1])
    # print('updateion lastSerialNo',main.POCW.lastSerialNo)
    ...
    try:
        ...
        # if (data['Token'] in x ):
         #   print("2")
            ...
            # print(main.marketW.table)
    # totaolmtm=np.sum(main.POCW.table[:,15])
    # main.label.setText('%.2f'%totaolmtm)
...
def updateLTP_CMPOCW(main,data):
    ...
    try:
        if (main.CMPOCW.model.lastSerialNo != 0):
            # print('ddd')
            #print("1")
            # x = (np.unique(main.CMPOCW.table[:main.CMPOCW.lastSerialNo, 2]))


            # if (data['Token'] in x ):
             #   print("2")
            fltr = np.asarray([data['Token']])
            x = main.CMPOCW.table[np.in1d(main.CMPOCW.table[:, 2], fltr), 9]
            # print(x)

            for i in x:
                netValue = main.CMPOCW.table[i, 6]
                qty = main.CMPOCW.table[i, 5]
                # print(type(qty),type(data['LTP']))
...
def updateCashIndex(self, data):
    token = data['Token']
    ltp = data['LTP']
    pc = data['%CH']

    #print('in updateCashIndex', data)
    if (token == 26000):
        self.PrcNFT.setText('%.2f' % ltp)
        self.percentageNFT.setText( pc)

    elif (token == 26001):
        self.PrcBNF.setText('%.2f' % ltp)
        self.percentageBNF.setText( pc)

    elif (token == 26002):
        self.PrcVIX.setText('%.2f' % ltp)
        self.percentageVIX.setText( pc)



def UpdateLTP_MW(self, a,st=7208):
    try:
            ...
            if (a['Token'] in x):
             #   print("2")
                ...
                for i in x:
                    ...
                    if(st == 7208):
                        ch = data['LTP'] - data['CLOSE']
                        self.marketW.table[i, editableList] = [a['LTP'], data['Bid'], a['Ask'], ch, a['%CH'], mtm,a['OPEN'],a['HIGH'],a['LOW'],a['CLOSE']]
                    else:
                        editableList = [9,15]
                        self.marketW.table[i, editableList] = [a['LTP'],  mtm]
...
def UpdateLTP_MW_basic(self, a,st = 7208):
    try:
        # print('in marketwatch update' ,a)
            ...
            if (a['Token'] in x):
                ...
                for i in x:
                    ...
                    if(st == 7208):
                        ...
                        editableList =[9]
                        self.marketWB.table[i, editableList] = [a['LTP']]
...
def UpdateLTP_NP(self, data):
    try:
        ...
        totaolmtm = np.sum(self.NetPos.Apipos[:, 10])
        self.NetPos.lbMTM.setText('%.2f' % totaolmtm)

    except:
        print(traceback.print_exc())

def updateSwapOrder(self,data):
    # if self.CFrame.dockMW.visibleRegion().isEmpty() == False:

    if(self.swapOrder.visibleRegion().isEmpty() == False):

        if(data['Token'] == self.swapOrder.tokenBuy):
            self.swapOrder.lbLTPBuy.setText('%.2f'%data['LTP'])
        elif(data['Token'] == self.swapOrder.tokenSell):
            self.swapOrder.lbLTPSell.setText('%.2f' % data['LTP'])
        else:
            pass
...
def UpdateLTP_FP(self, data):
    try:
        ...
        filterPos = self.FolioPos.table[np.where(self.FolioPos.table[:,3]==self.FolioPos.selectedFolio)]
        totaolmtm = np.sum(filterPos[:, 21])
        self.FolioPos.lbMTM.setText('%.2f' % totaolmtm)
    ...
    for i in pos:
        ...
        if(isRecordExist ==False):
            try:
                ...
                self.recv_fo.subscribedlist("MarketWatch",exchange,token)
...
def update_Position_Socket_NP(main,pos):
    for i in pos:
        isRecordExist = False
        fltr0 = np.asarray([i[3]])
        filteredArray0 =  main.NetPos.Apipos[np.in1d(main.NetPos.Apipos[:, 3], fltr0)]
        if (filteredArray0.size != 0):
            fltr = np.asarray([i[2]])
        ...
        if(isRecordExist ==False):
            try:
                exchange = i[2]
                token = i[3]
                pos[0,18] = main.NetPos.lastSerialNo
                main.NetPos.Apipos[main.NetPos.lastSerialNo, :] = pos
                main.NetPos.lastSerialNo +=1
                main.NetPos.modelP.lastSerialNo += 1
                main.NetPos.modelP.rowCount()
                main.NetPos.modelP.insertRows()
                ind = main.NetPos.modelP.index(0, 0)
                ind1 = main.NetPos.modelP.index(0, 1)
                main.NetPos.modelP.dataChanged.emit(ind, ind1)
                th1 = threading.Thread(target=subscribeToken,
                                       args=(main, token, exchange, 1501))
                th1.start()
            ...
            main.NetPos.Apipos[serialNo,[21,9,13]] = [day,net,netValue]
