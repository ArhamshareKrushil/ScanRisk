...
from Application.Utils.createTables import tables_details_CMPOCW


class UI_CMPOCW(QMainWindow):
    ################################# Intialization Here ##################################################
    def __init__(self):

        super(UI_CMPOCW, self).__init__()
        self.osType = platform.system()
        #########################################################
        ########################################################
        #########################################################
        loc1 = os.getcwd().split('Application')
        ui_login = os.path.join(loc1[0], 'Resources', 'UI', 'CMPOCW.ui')
    ...
    def saveAsDefaultColumnProfile(self):
        try:
            loc1 = os.getcwd().split('Application')
            defaultDir = os.path.join(loc1[0], 'Resources', 'CMPOCW_ColPro', 'ColumnProfile')

            binData = self.tableView.horizontalHeader().saveState()
            save = QFileDialog.getSaveFileName(self, 'Save file', defaultDir)[0]
            print('save CMPOCW save column profile', save)
            ...
            f1.close()
            pathDetails['CMPOCW']['defaultColumnProfile'] = save
    ...
    def defaultColumnProfile(self):
        ...
        pathDetails = json.load(f1)
        f1.close()
        lastCPFilePath = pathDetails['CMPOCW']['defaultColumnProfile']
    ...
    def saveColumnProfile(self):
        try:
            loc = os.getcwd().split('Application')
            defaultDir = os.path.join(loc[0], 'Resources', 'CMPOCW_ColPro', 'ColumnProfile')
            ...
            pathDetails = json.load(f1)
            f1.close()
            pathDetails['CMPOCW']['lastSavedColumnProfile'] = save
