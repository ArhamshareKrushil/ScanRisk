...
from Application.Utils.createTables import tables_details_CMTWM


class UI_CMTWM(QMainWindow):
    ################################# Intialization Here ##################################################
    def __init__(self):

        super(UI_CMTWM, self).__init__()
        self.osType = platform.system()
        #########################################################
        ########################################################
        #########################################################
        loc1 = os.getcwd().split('Application')
        ui_login = os.path.join(loc1[0], 'Resources', 'UI', 'CMTWM.ui')
    ...
    def saveAsDefaultColumnProfile(self):
        try:
            ...
            f1.close()
            pathDetails['CMTWM']['defaultColumnProfile'] = save
    ...
    def defaultColumnProfile(self):
        ...
        pathDetails = json.load(f1)
        f1.close()
        lastCPFilePath = pathDetails['CMTWM']['defaultColumnProfile']
    ...
    def saveColumnProfile(self):
        try:
            loc = os.getcwd().split('Application')
            defaultDir = os.path.join(loc[0], 'Resources', 'CMTWM_ColPro', 'ColumnProfile')
            ...
            pathDetails = json.load(f1)
            f1.close()
            pathDetails['CMTWM']['lastSavedColumnProfile'] = save
