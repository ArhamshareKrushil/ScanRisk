...
from Application.Utils.createTables import tables_details_TWSWM


class TScriptSummary(QMainWindow):
    ################################# Intialization Here ##################################################
    def __init__(self):

        super(TScriptSummary, self).__init__()
        self.osType = platform.system()
        #########################################################
        ########################################################
        #########################################################
        loc1 = os.getcwd().split('Application')
        ui_login = os.path.join(loc1[0], 'Resources', 'UI', 'tableTWSWM.ui')
    ...
    def saveAsDefaultColumnProfile(self):
        try:
            ...
            f1.close()
            pathDetails['TWSWM']['defaultColumnProfile'] = save
    ...
    def defaultColumnProfile(self):
        ...
        pathDetails = json.load(f1)
        f1.close()
        lastCPFilePath = pathDetails['TWSWM']['defaultColumnProfile']
    ...
    def saveColumnProfile(self):
        try:
            loc = os.getcwd().split('Application')
            defaultDir = os.path.join(loc[0], 'Resources', 'TWSWM_ColPro', 'ColumnProfile')
            # defaultDir = r'../Resources/TWSWM_ColPro/ColumnProfile'
            ...
            pathDetails = json.load(f1)
            f1.close()
            pathDetails['TWSWM']['lastSavedColumnProfile'] = save
