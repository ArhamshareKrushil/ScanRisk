...
from Application.Utils.createTables import tables_details_Tmaster

loc = os.getcwd().split('Application')[0]


class UI_Tmaster(QMainWindow):
    sgupdateTWMwithTmaster=pyqtSignal(dict)

    ################################# Intialization Here ##################################################
    def __init__(self):

        super(UI_Tmaster, self).__init__()
        self.osType = platform.system()
        #########################################################
        ########################################################
        #########################################################
        loc1 = os.getcwd().split('Application')
        ui_login = os.path.join(loc1[0], 'Resources', 'UI', 'terminalMaster.ui')
        ...
        self.title = tBar('Terminal Master')
