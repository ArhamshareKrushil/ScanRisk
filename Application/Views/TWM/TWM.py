...
from Application.Utils.createTables import tables_details_TWM


class TerminalSummary(QMainWindow):
    ################################# Intialization Here ##################################################
    def __init__(self):

        super(TerminalSummary, self).__init__()
        self.osType = platform.system()
        #########################################################
        ########################################################
        #########################################################
        loc1 = os.getcwd().split('Application')
        ui_login = os.path.join(loc1[0], 'Resources', 'UI', 'tableTWM.ui')
    ...
    def saveAsDefaultColumnProfile(self):
        try:
            ...
            f1.close()
            pathDetails['TWM']['defaultColumnProfile'] = save
    ...
    def defaultColumnProfile(self):
        ...
        pathDetails = json.load(f1)
        f1.close()
        lastCPFilePath = pathDetails['TWM']['defaultColumnProfile']
    ...
    def saveColumnProfile(self):
        try:
            loc = os.getcwd().split('Application')
            defaultDir = os.path.join(loc[0], 'Resources', 'TWM_ColPro', 'ColumnProfile')
            # defaultDir = r'../Resources/TWM_ColPro/ColumnProfile'
            ...
            pathDetails = json.load(f1)
            f1.close()
            pathDetails['TWM']['lastSavedColumnProfile'] = save
