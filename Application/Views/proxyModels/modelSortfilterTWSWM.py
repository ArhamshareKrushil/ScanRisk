...
class ProxyModel (QSortFilterProxyModel): #Custom Proxy Model
    ...
    def filterAcceptsRow(self, row, parent):
        ...
        if(self.cllientCode=='' and self.symbol==''):
            ...
            if (self.symbol.lower() in self.sourceModel().index(row, 1, parent).data().lower()):
                return True
            else:
                return False

        else:
            if(self.sourceModel().index(row, 0, parent).data() == self.cllientCode and self.symbol.lower() in self.sourceModel().index(row, 1, parent).data().lower()):
