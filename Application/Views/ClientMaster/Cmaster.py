...
from Application.Utils.createTables import tables_details_Cmaster

loc = os.getcwd().split('Application')[0]


class UI_Cmaster(QMainWindow):
    ################################# Intialization Here ##################################################
    def __init__(self):

        super(UI_Cmaster, self).__init__()
        self.osType = platform.system()
        #########################################################
        ########################################################
        #########################################################
        loc1 = os.getcwd().split('Application')
        ui_login = os.path.join(loc1[0], 'Resources', 'UI', 'clientMaster.ui')
    ...
    def Updaterow(self):
        st=time.time()
        path = os.path.join(loc, 'Clientmasternew2.csv')
        np.savetxt(path, self.table[:self.lastSerialNo], delimiter=",", header='ClientID,Name,Branch,EPS,ITR,BM,GRP,Active', fmt='%s')
        et=time.time()
        print('time',et-st)
