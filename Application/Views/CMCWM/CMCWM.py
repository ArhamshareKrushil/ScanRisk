...
from Application.Utils.createTables import tables_details_CMCWM


class UI_CMCWM(QMainWindow):
    ################################# Intialization Here ##################################################
    def __init__(self):

        super(UI_CMCWM, self).__init__()
        self.osType = platform.system()
        #########################################################
        ########################################################
        #########################################################
        loc1 = os.getcwd().split('Application')
        ui_login = os.path.join(loc1[0], 'Resources', 'UI', 'CMCWM.ui')
    ...
    def saveAsDefaultColumnProfile(self):
        try:
            ...
            f1.close()
            pathDetails['CMCWM']['defaultColumnProfile'] = save
    ...
    def defaultColumnProfile(self):
        ...
        pathDetails = json.load(f1)
        f1.close()
        lastCPFilePath = pathDetails['CMCWM']['defaultColumnProfile']
    ...
    def saveColumnProfile(self):
        try:
            loc = os.getcwd().split('Application')
            defaultDir = os.path.join(loc[0], 'Resources', 'CMCWM_ColPro', 'ColumnProfile')
            ...
            pathDetails = json.load(f1)
            f1.close()
            pathDetails['CMCWM']['lastSavedColumnProfile'] = save
