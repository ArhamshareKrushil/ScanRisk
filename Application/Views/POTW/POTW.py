...
from Application.Utils.createTables import tables_details_POTW


class PositionDetailsTW(QMainWindow):
    ################################# Intialization Here ##################################################
    def __init__(self):

        super(PositionDetailsTW, self).__init__()
        self.osType = platform.system()
        #########################################################
        ########################################################
        #########################################################
        loc1 = os.getcwd().split('Application')
        ui_login = os.path.join(loc1[0], 'Resources', 'UI', 'tablePOTW.ui')
    ...
    def saveAsDefaultColumnProfile(self):
        try:
            ...
            f1.close()
            pathDetails['POTW']['defaultColumnProfile'] = save
    ...
    def defaultColumnProfile(self):
        ...
        pathDetails = json.load(f1)
        f1.close()
        lastCPFilePath = pathDetails['POTW']['defaultColumnProfile']
    ...
    def saveColumnProfile(self):
        try:
            loc = os.getcwd().split('Application')
            defaultDir = os.path.join(loc[0], 'Resources', 'POTW_ColPro', 'ColumnProfile')
            ...
            pathDetails = json.load(f1)
            f1.close()
            pathDetails['POTW']['lastSavedColumnProfile'] = save
