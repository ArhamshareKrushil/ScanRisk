...
def process(self,Decoded_Packet,Start_Pointof_Record):
    ...
    if Token in self.FinalList['NSEFO']:
        ...
        for rb in range(10):
            ...
            Price /= 100.0
            buffer_output = {'Quantity': Quantity, 'Price': round(Price,2), 'NumberOfOrders': NumberOfOrders,
        ...
        # output1 = {'Exch':2, 'Token': Token, 'symbol':symbol,'BookType': BookType,
        #            'TradingStatus': TradingStatus,
        #           'Volume': Volume, 'LTP': LTP,'Bid':Bids[0]['Price'] ,'BQ':Bids[0]['Quantity'],'Ask':Asks[0]['Price'],'AQ':Bids[0]['Quantity'],
        #           'NetChangeIndicator': NetChangeIndicator, 'NetPriceChangeFromClosingPrice': str(LTP - NetPriceChangeFromClosingPrice),
        #           'LTQ':LTQ, 'LastTradeTime':LastTradeTime, 'ATP':ATP,
        #           'AuctionNumber':AuctionNumber, 'AuctionStatus':AuctionStatus,'InitiatorType':InitiatorType,'InitiatorPrice':InitiatorPrice,
        #           'InitiatorQuantity':InitiatorQuantity, 'AuctionPrice':AuctionPrice, 'AuctionQuantity':AuctionQuantity ,'BbTotalBuyFlag':BbTotalBuyFlag,
        ...
        output1 = {'ID': 1501, 'Exch': 2, 'Token': Token, 'symbol': symbol, 'BookType': BookType,
                   ...
                   'NetChangeIndicator': NetChangeIndicator, '%CH': '%.2f'%(LTP - NetPriceChangeFromClosingPrice),
                   'LTQ': LTQ, 'LTT': LastTradeTime, 'ATP': ATP, 'CLOSE': CLOSE,
                   'OPEN': OPEN, 'HIGH': HIGH, 'LOW': LOW}

        # x = json.dumps(output1).encode()
