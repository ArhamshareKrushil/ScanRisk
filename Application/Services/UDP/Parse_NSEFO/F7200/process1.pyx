...
def process(self,Decoded_Packet,Start_Pointof_Record):
    ...
    InitiatorPrice, InitiatorQuantity, AuctionPrice, AuctionQuantity = \
        struct.unpack('!lhhll2sllllhhhllll', Decoded_Packet[Start_Pointof_Record:Start_Pointof_Record + 56])
    ...
    MBO_buffer_sp = Start_Pointof_Record + 56
    for rb in range(10):
    ...
    Start_Pointof_Record = MBP_buffer_sp

    Total_Buy_Quantity = struct.unpack('!d', Decoded_Packet[Start_Pointof_Record:Start_Pointof_Record + 8])
    Total_Sell_Quantity = struct.unpack('!d', Decoded_Packet[Start_Pointof_Record + 8:MBO_buffer_sp + 16])

    # STINDICATOR

    CLOSE, OPEN, HIGH, LOW = struct.unpack('!llll', Decoded_Packet[
                                                                          Start_Pointof_Record + 18:Start_Pointof_Record + 34])

    output = {'ID': 7200,'Exch':2, 'Token': Token, 'BookType': BookType, 'TradingStatus': TradingStatus,
              ...
              'Total_Sell_Quantity': Total_Sell_Quantity, 'CLOSE': CLOSE, 'OPEN': OPEN,'HIGH': HIGH, 'LOW': LOW,
              'Bids_MBO':Bids_MBO,'Asks_MBO':Asks_MBO,'Bids_MBP':Bids_MBP,'Asks_MBP':Asks_MBP
              }
    return output
